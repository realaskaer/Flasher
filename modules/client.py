...
from config import ERC20_ABI, CHAIN_IDS, TOKENS_PER_CHAIN, COINGECKO_TOKEN_API_NAMES
from web3 import AsyncHTTPProvider, AsyncWeb3
from config import RHINO_CHAIN_INFO, ORBITER_CHAINS_INFO, LAYERSWAP_CHAIN_NAME
from settings import (
    GAS_LIMIT_MULTIPLIER,
    GAS_PRICE_MULTIPLIER, UNLIMITED_APPROVE, SLIPPAGE, WAIT_FOR_RECEIPT_L0,
)
...
class Client(Logger):
    def __init__(
            self, account_name: str | int, private_key: str, network: Network, proxy: None | str = None
    ):
    ...
    async def get_smart_amount(
            self, settings: tuple, need_percent: bool = False, token_name: str = None,
            omnicheck: bool = False, fee_support: float = None
    ) -> float:
        if not token_name:
            ...
            try:
                ...
                while True:
                    ...
                    if new_eth_balance > old_balance:
                        decimals = 18 if client.network.name != 'Solana' else 9
                        ...
                        amount = client.custom_round((new_eth_balance - old_balance) / 10 ** decimals, 6)
                        client.logger_msg(
                            *client.acc_info, msg=f'{amount} {token_name} was received on {client.network.name}',
                            type_msg='success'
                        )
                        return True
                    else:
    ...
    async def get_token_balance(
            ...
            check_native: bool = False, token_address: str = None, without_error: bool = False
    ) -> [int, float, str]:
        if not token_name:
            token_name = self.token

        if token_name in ['MAV', 'STG', 'TIA.n', 'USDV']:
            omnicheck = True
    ...
    async def prepare_transaction(self, value: int = 0) -> dict:
        try:
            ...
            if self.network.eip1559_support:
                ...
                max_fee_per_gas = int(base_fee + max_priority_fee_per_gas * 1.4 * GAS_PRICE_MULTIPLIER)

                if self.network.name == ['Scroll', 'Optimism']:
                    max_fee_per_gas = int(max_fee_per_gas / GAS_PRICE_MULTIPLIER * 1.3)
    ...
    async def wait_for_l0_received(self, tx_hash: HexStr, need_wait: bool = False) -> bool:
        if not WAIT_FOR_RECEIPT_L0 and not need_wait:
            return True
